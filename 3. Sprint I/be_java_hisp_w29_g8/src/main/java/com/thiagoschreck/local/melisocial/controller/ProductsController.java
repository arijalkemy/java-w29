...
public class ProductsController {
    ...
    @PostMapping("/post")
    public ResponseEntity<CreatePostDTO> create(@RequestBody CreatePostDTO createPostDto) {
        return ResponseEntity.ok(postsService.saveWithoutDiscount(createPostDto));
    }

    @PostMapping("/promo-post")
    public ResponseEntity<CreatePostDTO> createDiscountedPost(@RequestBody CreatePostDTO createPostDto) {
    ...
    public ResponseEntity<List<FeedPostsDto>> getAllPostsByFollowedSellerWithinTwoWeeksOrder(
            @PathVariable Integer userId, @RequestParam(defaultValue = "date_asc") String order) {

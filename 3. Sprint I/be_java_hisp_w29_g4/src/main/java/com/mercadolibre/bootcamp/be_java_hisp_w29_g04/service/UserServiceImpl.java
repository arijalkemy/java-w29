...
import com.mercadolibre.bootcamp.be_java_hisp_w29_g04.utils.UserValidation;
...
public class UserServiceImpl implements IUserService {
    ...
    private boolean checkFollow(User follower, User followed) {
    ...
    public MessageDto unfollowSeller (Integer userId, Integer sellerId) {
        User commonUser = findById(userId);
        User seller = findById(sellerId);
        if (!UserValidation.isSeller(seller)) throw new InvalidRelationshipException("Invalid seller.");
        if (!UserValidation.isCommonUser(commonUser)) throw new InvalidRelationshipException("Invalid user id");
     ...
     * **/
    @Override
    public FollowersDto getUsersBySeller(Integer userId, UserOrderTypeEnum orderType) {
        User seller = findById(userId);
        if (!UserValidation.isSeller(seller)) throw new InvalidRelationshipException("Only sellers have followers");
        ...
        return new FollowersDto(
                seller.getUserId(),
                seller.getUsername(),
                getFollowUsers(followers, orderType)
        );
    }
     ...
     * **/
    @Override
    public FollowingDto getSellersByUser(Integer userId, UserOrderTypeEnum orderType) {
        ...
        return new FollowingDto(
                ...
                getFollowUsers(following, orderType)
        );
    }

...
public class Main {
    public static void main(String[] args) {
        ...
        for (Integer pId : participantsById.keySet()) {
            ...
            if (c.getFirst().equals(categoryNumber)) {
        ...
        categoryNumber = categoriesByParticipantId.get(participantId).getFirst();
        ...
        for (Integer pId : participantsById.keySet()) {
            ...
            switch (c.getFirst()) {
                case "1":
                    prices[Integer.parseInt(c.getFirst()) - 1] += pAge < 18 ? 1300 : 1500;
                    break;
                case "2":
                    prices[Integer.parseInt(c.getFirst()) - 1] += pAge < 18 ? 2000 : 2300;
                    break;
                case "3":
                    if (pAge < 18) {
                        System.out.println("Participante " + pId + " no puede inscribirse: Menor de 18 años.");
                        break;
                    }
                    prices[Integer.parseInt(c.getFirst()) - 1] += 2800;
                    break;
            }
        }

        for (int categoryIndex = 0; categoryIndex < prices.length; categoryIndex++) {
            print("Category " + categoryIndex + ": " + prices[categoryIndex]);
        }

...
/*
 ...
 //2
 SELECT p.velocidad_megas, COUNT(c.dni) AS total_clientes FROM planes_internet p
...
FROM planes_internet;
 ...
 //5
 SELECT c.nombre, c.apellido, p.descripcion FROM clientes c
JOIN planes_internet p ON c.id_plan = p.id_plan
WHERE p.descuento > 15;

//6
 SELECT c.nombre, c.apellido, p.velocidad_megas FROM clientes c
JOIN planes_internet p ON c.id_plan = p.id_plan;

//7
 SELECT provincia, COUNT(*) AS total_clientes FROM clientes
GROUP BY provincia;
...
// 9
SELECT velocidad_megas,precio FROM planes_internet
WHERE descuento > 10;
